...
/**
 * Keep centered dialogs visible above the soft keyboard (iOS overlay / mobile web).
 * Returns inline positioning when lifted; pair with lifted class overrides on DialogContent.
 */
export function useKeyboardAwareDialogPosition(enabled: boolean): {
  style?: CSSProperties;
  lifted: boolean;
} {
  const keyboardOpen = useKeyboardOpen(enabled);
  const keyboardInset = useKeyboardInset(enabled);
  const viewportBox = useVisualViewportBox(enabled, false);

  const lifted =
    enabled &&
    keyboardOpen &&
    (keyboardInset > 0 ||
      viewportBox.height < viewportBox.layoutHeight - RESIZED_MATCH_PX);
