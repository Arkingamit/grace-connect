...
export interface FlipCardItem {
  id: string;
  type: 'event' | 'announcement' | 'prayer' | 'sermon' | 'worship_video' | 'note' | 'custom';
  itemId?: string;
  title?: string;
  description?: string;
  buttonText?: string;
  buttonLink?: string;
  /** Audience targeting (same pattern as events / announcements) */
  targetCampuses?: string[];
  targetGroups?: string[];
  excludeCampuses?: string[];
  excludeGroups?: string[];
  /** 0 = Forever; otherwise hours until expiry */
  highlightDurationHours?: number;
  highlightExpiresAt?: string | null;
}
